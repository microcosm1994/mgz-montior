<template>
  <div class="cpdup">
        ...
        <form>
          <fieldset>
            ...
            <ul>
              <li class="li-span-2">
                <label>在线状态</label>
                <span :class="Status.StationUP_state_report.onLine"></span>
              </li>
              <li class="li-span-2">
                <label>本分控状态</label>
                <input type="text" v-model="Status.StationUP_state_report.State" disabled>
              </li>
              <li class="li-span-2">
                <label>自检状态</label>
                <input type="text" v-model="Status.StationUP_state_report.examination_state" disabled>
              </li>
              <li class="li-span-2">
                <label>衰减</label>
                <input type="text" v-model="Status.StationUP_state_report.attenuation" disabled>
              </li>
              <li class="li-span-2">
                <label>群时延本振测试</label>
                <input type="text" v-model="Status.StationUP_state_report.loTest" disabled>
              </li>
              <li class="li-span-2">
                <label>本振锁定状态</label>
                <input type="text" v-model="Status.StationUP_state_report.Local_locking" disabled>
              </li>
              <li class="li-span-2">
                <label>内外参考状态</label>
                <input type="text" v-model="Status.StationUP_state_report.reference" disabled>
              </li>
              <li class="li-span-2">
                <label>参考锁定状态</label>
                <input type="text" v-model="Status.StationUP_state_report.lock_state" disabled>
              </li>
              <li class="li-span-2">
                <label>温度</label>
                <input type="text" v-model="Status.StationUP_state_report.temperature" disabled>
              </li>
              <li class="li-span-2">
                <label>湿度</label>
                <input type="text" v-model="Status.StationUP_state_report.humidity" disabled>
              </li>
            </ul>
          </fieldset>
        </form>
    ...
    <el-dialog
      ...
      width="50%"
      center>
      <cpdup_modal></cpdup_modal>
    </el-dialog>
  </div>
</template>

<script>
import cpdup_modal from './cpdup_modal'
export default {
  name: 'cpdup',
  data () {
    return {
  ...
  computed: {
    changTitle: function () {
      ...
      return 'C频段上变频器 (' + data.name + ')'
    },
    device: function () {
      let data = this.$route.query
      if (data.name === 'A') {
        return this.$device.Station_UP_1
      }
      if (data.name === 'B') {
        return this.$device.Station_UP_2
      }
    },
    // 获取设备id和key数组、结构体信息
    structGet: function () {
