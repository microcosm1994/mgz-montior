      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
              ...
              <div class="search-box">
                <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
                  <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
                    ...
                    <el-option label="工作方式名称" value="name"></el-option>
                    <el-option label="" value="linkMacro_id"></el-option>
                    <el-option label="" value="parameterMacro_id"></el-option>
                    <el-option label="创建时间" value="createTime"></el-option>
                    <el-option label="修改时间" value="updateTime"></el-option>
                    <el-option label="1 已删除;0 未删除" value="deleted"></el-option>
                  </el-select>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添工作方式表</el-button>
            </el-col>
          </el-row>
        </div>
        <el-table :data="tableData" :header-row-style="{height: '70px'}" border style="width: 100%" height="70vh" :stripe="true">
          ...
          <el-table-column  prop="name"   label="工作方式名称"> </el-table-column>
          <el-table-column  prop="linkMacro_id"   label=""> </el-table-column>
          <el-table-column  prop="parameterMacro_id"   label=""> </el-table-column>
        ...
        <el-dialog title="新添工作方式表" :visible="dialog" width="40%" :before-close="createClose">
          <el-form ref="dialogData" :model="dialogData" label-width="140px">
            <el-row>
              <el-col :span="24">
                <el-form-item label="工作方式名称">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="">
                  <el-input v-model="dialogData.linkMacro_id"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item label="">
                  <el-input v-model="dialogData.parameterMacro_id"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/mgz/workWay/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id - 0 === deviceData[i].parent_id) {
              this.$http.post('/api/mgz/workWay/get', {parent_id: id, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/mgz/workWay/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/mgz/workWay/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        let data = {
        	...
        	linkMacro_id: this.dialogData.linkMacro_id,
        	parameterMacro_id: this.dialogData.parameterMacro_id
        }
        // 新添
        if (this.saveType === 'create') {
          this.$http.post('/api/mgz/workWay/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/mgz/workWay/put', data).then((response) => {
            if (response.status === 200) {
