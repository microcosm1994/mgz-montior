      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
              ...
              <div class="search-box">
                <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
                  <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
                    ...
                    <el-option label="宏名称" value="name"></el-option>
                    <el-option label="宏文件类型 1：参数宏；2工作方式宏" value="type"></el-option>
                    <el-option label="创建时间" value="createTime"></el-option>
                    <el-option label="修改时间" value="updateTime"></el-option>
                    <el-option label="是否被删除 1删除 0可用" value="deleted"></el-option>
                    <el-option label="描述" value="describe"></el-option>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添宏文件</el-button>
            </el-col>
          </el-row>
        </div>
        <el-table :data="tableData" :header-row-style="{height: '70px'}" border style="width: 100%" height="70vh" :stripe="true">
          <el-table-column type="index" width="50"></el-table-column>
          <el-table-column  prop="id"   label="宏编号"> </el-table-column>
          <el-table-column  prop="name"   label="宏名称"> </el-table-column>
          <el-table-column  prop="type"   label="宏文件类型 1：参数宏；2工作方式宏"> </el-table-column>
          <el-table-column  prop="createTime"   label="创建时间"> </el-table-column>
          <el-table-column  prop="updateTime"   label="修改时间"> </el-table-column>
          <el-table-column  prop="describe"   label="描述"> </el-table-column>
          <el-table-column  prop="taskCode"   label="任务代号"> </el-table-column>
          <el-table-column  prop="taskMark"   label="任务标识"> </el-table-column>
          <el-table-column fixed="right"  label="操作" width="150px">
            <template slot-scope="scope">
              
                <el-button @click="jump(scope.row, '/mgz/workWay/get')" type="text" size="small">工作方式表</el-button>
                <!--<el-button type="text" size="small">-->
                  <!--<a href="#/mgz/workWay/get?linkMacro_id=" target="_blank">工作方式表</a>-->
                <!--</el-button>-->
        ...
        <el-dialog title="新添宏文件" :visible="dialog" width="40%" :before-close="createClose">
          <el-form ref="dialogData" :model="dialogData" label-width="140px">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="宏文件类型 1：参数宏；2工作方式宏">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="任务代号">
                  <el-input v-model="dialogData.taskCode"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item label="任务标识">
                  <el-input v-model="dialogData.taskMark"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/mgz/macro/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id - 0 === deviceData[i].parent_id) {
              this.$http.post('/api/mgz/macro/get', {parent_id: id, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/mgz/macro/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/mgz/macro/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        let data = {
        	...
        	taskCode: this.dialogData.taskCode,
        	taskMark: this.dialogData.taskMark
        }
        // 新添
        if (this.saveType === 'create') {
          this.$http.post('/api/mgz/macro/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/mgz/macro/put', data).then((response) => {
            if (response.status === 200) {
