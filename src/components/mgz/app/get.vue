      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
              ...
              <div class="search-box">
                <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
                  <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
                    ...
                    <el-option label="脚本文件路径" value="scriptFilename"></el-option>
                    <el-option label="创建时间" value="createTime"></el-option>
                    <el-option label="修改时间" value="updateTime"></el-option>
                    <el-option label="是否被删除 1已删除 0可用" value="deleted"></el-option>
                  </el-select>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添版本表</el-button>
            </el-col>
          </el-row>
        </div>
        <el-table :data="tableData" :header-row-style="{height: '70px'}" border style="width: 100%" height="70vh" :stripe="true">
          ...
          <el-table-column  prop="scriptFilename"   label="脚本文件路径"> </el-table-column>
          <el-table-column  prop="createTime"   label="创建时间"> </el-table-column>
          <el-table-column  prop="updateTime"   label="修改时间"> </el-table-column>
          <el-table-column fixed="right"  label="操作" width="150px">
            <template slot-scope="scope">
              
                <el-button @click="jump(scope.row, '/mgz/appFile/get')" type="text" size="small">结构体路径表</el-button>
                <!--<el-button type="text" size="small">-->
                  <!--<a href="#/mgz/appFile/get?app_id=" target="_blank">结构体路径表</a>-->
                <!--</el-button>-->
        ...
        <el-dialog title="新添版本表" :visible="dialog" width="40%" :before-close="createClose">
          <el-form ref="dialogData" :model="dialogData" label-width="140px">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="脚本文件路径">
                  <el-input v-model="dialogData.scriptFilename"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/mgz/app/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id - 0 === deviceData[i].parent_id) {
              this.$http.post('/api/mgz/app/get', {parent_id: id, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/mgz/app/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/mgz/app/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        let data = {
        	...
        	scriptFilename: this.dialogData.scriptFilename
        }
        // 新添
        if (this.saveType === 'create') {
          this.$http.post('/api/mgz/app/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/mgz/app/put', data).then((response) => {
            if (response.status === 200) {
