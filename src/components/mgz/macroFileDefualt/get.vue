  ...
  <el-card class="box-card" shadow="hover">
    <div slot="header" class="clearfix header-box">
      <el-row>
          ...
          <div class="search-box">
            <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
              <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
             	...
             	<el-option label="默认宏配置名称" value="name"></el-option>
             	<el-option label="" value="uri"></el-option>
             	<el-option label="是否已删除" value="deleted"></el-option>
             	<el-option label="创建时间" value="createTime"></el-option>
             	<el-option label="修改时间" value="updateTime"></el-option>
             	<el-option label="" value="device_id"></el-option>
              </el-select>
        ...
        <el-col :span="3">
          <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添</el-button>
        </el-col>
      </el-row>
    </div>
    <el-table :data="tableData" border style="width: 100%" height="70vh">
      ...
      <el-table-column  prop="name"   label="默认宏配置名称"> </el-table-column>
      <el-table-column  prop="uri"   label=""> </el-table-column>
      <el-table-column  prop="createTime"   label="创建时间"> </el-table-column>
      <el-table-column  prop="updateTime"   label="修改时间"> </el-table-column>
      <el-table-column  prop="device_id"   label=""> </el-table-column>
    ...
    <el-dialog title="新添" :visible="dialog" width="40%" :before-close="createClose">
      <el-form ref="dialogData" :model="dialogData" label-width="140px">
     	<el-row>
          <el-col :span="24">
            <el-form-item label="默认宏配置名称">
     	...
     	<el-row>
          <el-col :span="24">
            <el-form-item label="">
              <el-input v-model="dialogData.device_id"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
<script>
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/mgz/macroFileDefualt/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id === deviceData[i].id) {
              ...
              this.$http.post('/api/mgz/macroFileDefualt/get', {id: deviceData[i].$device_struct_ibfk_2, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/mgz/macroFileDefualt/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/mgz/macroFileDefualt/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        let data = {
        	...
        	device_id: this.dialogData.device_id
        }
        // 新添
        if (this.saveType === 'create') {
          this.$http.post('/api/mgz/macroFileDefualt/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/mgz/macroFileDefualt/put', data).then((response) => {
            if (response.status === 200) {
