      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
              ...
              <div class="search-box">
                <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
                  <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
                    ...
                    <el-option label="宏编号" value="macro_id"></el-option>
                    <el-option label="文件访问路径" value="uri"></el-option>
                    <el-option label="创建时间" value="createTime"></el-option>
                    <el-option label="修改时间" value="updateTime"></el-option>
                    <el-option label="是否被删除 1删除 0可用" value="deleted"></el-option>
                    <el-option label="设备id" value="device_id"></el-option>
                  </el-select>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添结构体路径表</el-button>
            </el-col>
          </el-row>
        </div>
        <el-table :data="tableData" :header-row-style="{height: '70px'}" border style="width: 100%" height="70vh" :stripe="true">
          ...
          <el-table-column  prop="macro_id"   label="宏编号"> </el-table-column>
          <el-table-column  prop="uri"   label="文件访问路径"> </el-table-column>
        ...
        <el-dialog title="新添结构体路径表" :visible="dialog" width="40%" :before-close="createClose">
          <el-form ref="dialogData" :model="dialogData" label-width="140px">
            <el-row>
              <el-col :span="24">
                <el-form-item label="宏编号">
                  <el-input v-model="dialogData.macro_id"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/mgz/macroFile/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id - 0 === deviceData[i].parent_id) {
              this.$http.post('/api/mgz/macroFile/get', {parent_id: id, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/mgz/macroFile/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/mgz/macroFile/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        let data = {
        	...
        	macro_id: this.dialogData.macro_id,
        	uri: this.dialogData.uri,
        	device_id: this.dialogData.device_id
        }
        // 新添
        if (this.saveType === 'create') {
          this.$http.post('/api/mgz/macroFile/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/mgz/macroFile/put', data).then((response) => {
            if (response.status === 200) {
      ...
      // 删除
      remove (row) {
        this.$confirm('是否删除这个结构体路径表？', '确认信息', {
