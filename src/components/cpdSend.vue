<template>
  <div class="cpdSend">
    <el-tabs type="border-card">
      ...
      <el-tab-pane label= "C频段发射校零变频器">
        <form>
          <fieldset>
            <legend>C频段发射校零变频器状态：</legend>
            <!--C频段发射校零变频器-->
            <ul>
              <li class="li-span-2">
                <label>在线状态</label>
                <span :class="Status.StationSendZero_state_report.onLine"></span>
              </li>
              <li class="li-span-2">
                <label>本分控状态</label>
                <input type="text" v-model="Status.StationSendZero_state_report.State" disabled>
              </li>
              <li class="li-span-2">
                <label>自检状态</label>
                <input type="text" v-model="Status.StationSendZero_state_report.examination_state" disabled>
              </li>
              <li class="li-span-2">
                <label>群时延本振测试</label>
                <input type="text" v-model="Status.StationSendZero_state_report.loTest" disabled>
              </li>
              <li class="li-span-2">
                <label>本振锁定状态</label>
                <input type="text" v-model="Status.StationSendZero_state_report.Local_locking" disabled>
              </li>
              <li class="li-span-2">
                <label>内外参考状态</label>
                <input type="text" v-model="Status.StationSendZero_state_report.reference" disabled>
              </li>
              <li class="li-span-2">
                <label>参考锁定状态</label>
                <input type="text" v-model="Status.StationSendZero_state_report.lock_state" disabled>
              </li>
              <li class="li-span-2">
                <label>射频输入状态</label>
                <input type="text" v-model="Status.StationSendZero_state_report.input" disabled>
              </li>
              <li class="li-span-2">
                <label>中频输出状态</label>
                <input type="text" v-model="Status.StationSendZero_state_report.output" disabled>
              </li>
              <li class="li-span-2">
                <label>温度</label>
                <input type="text" v-model="Status.StationSendZero_state_report.temperature" disabled>
              </li>
              <li class="li-span-2">
                <label>湿度</label>
                <input type="text" v-model="Status.StationSendZero_state_report.humidity" disabled>
              </li>
            </ul>
          </fieldset>
        </form>
    ...
    <el-dialog
      ...
      width="50%"
      center>
      <cpdSend_modal></cpdSend_modal>
    </el-dialog>
  </div>
</template>

<script>
import cpdSend_modal from './cpdSend_modal'
export default {
  name: 'cpdSend',
  data () {
    return {
  ...
  computed: {
    device: function () {
      return this.$device.StationSendZero
    },
    // 获取设备id和key数组、结构体信息
    structGet: function () {
      let keyArray = []
