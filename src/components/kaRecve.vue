  ...
  <div class="kaRecve">
    <el-tabs type="border-card">
     <div style="text-align: right;">
      ...
      <el-tab-pane label= 'ka频段接收校零变频器'>
        <form>
          <fieldset>
            <legend>ka频段接收校零变频器状态：</legend>
           <ul>
             <li class="li-span-2">
               <label>在线状态</label>
               <span :class="Status.RecvZero_state_report.onLine"></span>
             </li>
             <li class="li-span-2">
               <label>本远控状态</label>
               <input type="text" v-model="Status.RecvZero_state_report.State" disabled>
             </li>
             <li class="li-span-2">
               <label>自检状态</label>
               <input type="text" v-model="Status.RecvZero_state_report.examination_state" disabled>
             </li>
             <li class="li-span-2">
               <label>群时延本振测试</label>
               <input type="text" v-model="Status.RecvZero_state_report.loTest" disabled>
             </li>
             <li class="li-span-2">
               <label>本振模块锁定指示</label>
               <input type="text" v-model="Status.RecvZero_state_report.Local_locking" disabled>
             </li>
             <li class="li-span-2">
               <label>内外参考状态</label>
               <input type="text" v-model="Status.RecvZero_state_report.reference" disabled>
             </li>
             <li class="li-span-2">
               <label>参考锁定状态</label>
               <input type="text" v-model="Status.RecvZero_state_report.lock_state" disabled>
             </li>
             <li class="li-span-2">
               <label>射频输出状态</label>
               <input type="text" v-model="Status.RecvZero_state_report.Intermediate" disabled>
             </li>
             <li class="li-span-2">
               <label>温度</label>
               <input type="text" v-model="Status.RecvZero_state_report.temperature" disabled>
             </li>
             <li class="li-span-2">
               <label>湿度</label>
               <input type="text" v-model="Status.RecvZero_state_report.humidity" disabled>
             </li>
           </ul>
          </fieldset>
        </form>
    ...
    <el-dialog
      ...
      width="50%"
      center>
      <kaRecve_modal></kaRecve_modal>
    </el-dialog>
  </div>
</template>

<script>
import kaRecve_modal from './kaRecve_modal'
export default {
  name: 'kaRecve',
  data () {
    return {
      ...
      Status: {
        // Ka接收校零变频器状态
        RecvZero_parameter_setting: {},
        // Ka接收校零变频器参数设置
        RecvZero_state_report: {}
      }
    }
  },
  components: {
